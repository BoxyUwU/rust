...
use clippy_utils::ty::{is_type_lang_item, match_type};
...
fn get_ty_param(ty: Ty<'_>) -> Option<Ty<'_>> {
    if let ty::Adt(_, subs) = ty.kind() { subs.types().next() } else { None }
}
    ...
    if let ExprKind::Lit(lit) = &e.kind {
        ...
        if is_type_lang_item(cx, ty, LangItem::String)
            || (is_type_lang_item(cx, ty, LangItem::OwnedBox)
                && get_ty_param(ty).map_or(false, Ty::is_str))
            || (match_type(cx, ty, &paths::COW) && get_ty_param(ty).map_or(false, Ty::is_str))
        {
            Some(RepeatKind::String)
        } else {
            let ty = ty.peel_refs();
            (ty.is_str() || is_type_lang_item(cx, ty, LangItem::String)).then(|| RepeatKind::String)
        }
    }
}
