...
impl<'a, 'tcx> PatCtxt<'a, 'tcx> {
    ...
    fn lower_path(&mut self, qpath: &hir::QPath<'_>, id: hir::HirId, span: Span) -> Box<Pat<'tcx>> {
        ...
        let const_value = self
            ...
            .map(|val| match val {
                Some(valtree) => mir::Const::Valtree(valtree, ty),
                None => mir::Const::Val(
                    self.tcx
    ...
    fn lower_inline_const(
        ...
        if let Some(lit_input) = lit_input {
            match tcx.at(expr.span).lit_to_const(lit_input) {
                Ok(c) => return self.const_to_pat(Const::from_ty_const(c, ty, tcx), id, span).kind,
        ...
        {
            let subpattern = self.const_to_pat(Const::Valtree(valtree, ty), id, span);
    ...
    fn lower_lit(&mut self, expr: &'tcx hir::Expr<'tcx>) -> PatKind<'tcx> {
        ...
        match self.tcx.at(expr.span).lit_to_const(lit_input) {
            Ok(constant) => {
                self.const_to_pat(
                    Const::from_ty_const(constant, ct_ty, self.tcx),
                    expr.hir_id,
                    lit.span,
                )
                .kind
