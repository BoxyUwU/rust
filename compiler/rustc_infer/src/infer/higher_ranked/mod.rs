...
use rustc_middle::ty::{self, Binder, TyCtxt, TypeFoldable, TypeVisitableExt};
...
impl<'a, 'tcx> CombineFields<'a, 'tcx> {
        ...
        T: Relate<'tcx>,
    {
        ...
        self.infcx.enter_forall_binder(sup, |sup_prime| {
...
impl<'tcx> InferCtxt<'tcx> {
    ...
    pub fn enter_forall_binder<T, R>(
        &self,
        ...
        T: TypeFoldable<TyCtxt<'tcx>>,
    {
        if !binder.as_ref().skip_binder().has_escaping_bound_vars() {
            debug!("no bound vars");
            return f(binder.skip_binder());
        }

        self.enter_new_universe(|next_universe| {
            let delegate = FnMutDelegate {
                ...
                consts: &mut |bound_var: ty::BoundVar, ty| {
                    self.tcx.mk_const(
                        ty::PlaceholderConst { universe: next_universe, name: bound_var },
                        ty,
                    )
                },
            };
            ...
            debug!(?next_universe);
            let instantiated_binder = self.tcx.replace_bound_vars_uncached(binder, delegate);
            debug!(?instantiated_binder);
            f(instantiated_binder)
        })
    }
