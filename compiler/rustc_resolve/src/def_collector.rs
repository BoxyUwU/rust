...
impl<'a, 'ra, 'tcx> DefCollector<'a, 'ra, 'tcx> {
    ...
    fn with_const_arg_ctxt<F: FnOnce(&mut Self)>(&mut self, is_direct: bool, f: F) {
        let orig_cat = mem::replace(&mut self.invocation_parent.const_arg_ctxt, is_direct);
        f(self);
        self.invocation_parent.const_arg_ctxt = orig_cat;
    }
...
impl<'a, 'ra, 'tcx> visit::Visitor<'a> for DefCollector<'a, 'ra, 'tcx> {
    ...
    fn visit_anon_const(&mut self, constant: &'a AnonConst) {
        ...
        {
            self.with_const_arg_ctxt(true, |this| {
                visit::walk_anon_const(this, constant);
            });
        } else {
            self.with_const_arg_ctxt(false, |this| {
                let parent =
    ...
    fn visit_expr(&mut self, expr: &'a Expr) {
        let handle_const_block = |this: &mut Self, constant: &'a AnonConst, def_kind: DefKind| {
            ...
            this.with_const_arg_ctxt(false, |this| {
