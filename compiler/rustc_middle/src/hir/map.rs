...
impl<'tcx> TyCtxt<'tcx> {
    ...
    pub fn hir_const_arg_anon_check_invalid_param_uses(self, anon: LocalDefId) -> Result<(), ErrorGuaranteed> {
        ...
        impl<'tcx> Visitor<'tcx> for GenericParamVisitor<'tcx> {
            ...
            fn visit_path(&mut self, path: &crate::hir::Path<'tcx>, _id: HirId) -> ControlFlow<ErrorGuaranteed> {
                if let Res::Def(DefKind::TyParam | DefKind::ConstParam | DefKind::LifetimeParam, _) = path.res {
                    let e = self.0.dcx().struct_span_err(path.span, "generic parameters may not be used in const operations");
