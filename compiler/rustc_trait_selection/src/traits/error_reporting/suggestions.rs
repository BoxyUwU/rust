...
use rustc_middle::ty::{
    ...
    ToPredicate, Ty, TyCtxt, TypeFoldable, TypeFolder, TypeSuperFoldable, TypeVisitableExt,
    TypeckResults,
};
...
impl<'tcx> TypeErrCtxtExt<'tcx> for TypeErrCtxt<'_, 'tcx> {
            ...
            if let ty::Ref(region, base_ty, mutbl) = *real_ty.skip_binder().kind() {
                ...
                if let Some(steps) =
                    autoderef.into_iter().enumerate().find_map(|(steps, (ty, obligations))| {
                        // Re-add the `&`
                        let ty = self.tcx.mk_ref(region, ty, mutbl);
    ...
    fn maybe_suggest_convert_to_slice(
        ...
        let mut is_slice = |candidate: Ty<'tcx>| match *candidate.kind() {
            ty::RawPtr(ty::RawPtr { ty: t, mutbl: m }) | ty::Ref(_, t, m) => {
